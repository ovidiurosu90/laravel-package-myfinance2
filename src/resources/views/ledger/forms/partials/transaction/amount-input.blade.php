<div class="mb-3 required has-feedback row {{ $errors->has('amount') ?
                                                    'has-error' : '' }}">
    <label for="amount" class="col-5 control-label">
        {{ trans('myfinance2::ledger.forms.transaction-form.amount.label') }}
        <span id="amount_currency-label-tooltip" data-bs-toggle="tooltip"
    ...
    <div class="col-7 p-0 m-0 text-muted" id="calculated-amount"
        style="display: none">
        Calculated: <span></span>
    </div>
    <div class="col-12">
        <input type="number" step=".01" id="amount" name="amount"
            class="form-control" value="{{ $amount }}"
            placeholder="{{ trans('myfinance2::ledger.forms.transaction-form.amount.'
