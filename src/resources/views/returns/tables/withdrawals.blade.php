...
<tr class="{{ ($excludeDepositsWithdrawals ?? false) ? 'opacity-50' : '' }}">
    <td class="fw-bold">
        ...
        @if(($excludeDepositsWithdrawals ?? false) && ($excludeCash ?? false))
            @php
                $withdrawalsInfoTooltip = 'Cash movements (e.g. broker cash account to bank) are not counted'
                    . ' towards the return.'
    ...
    <td class="text-end fw-bold"
        ...
        @if(!($excludeDepositsWithdrawals ?? false))<span class="text-success">+</span>@endif
    </td>
