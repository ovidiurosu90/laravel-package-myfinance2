...
/**
 * Notification history for the Dip Buying Plan daily emails. Read-only append-only audit log; rows
 * can be deleted individually or in bulk.
 */
class DipBuyingNotificationController extends MyFinance2Controller
{
    ...
    {
        $items = DipBuyingNotification::where('user_id', auth()->user()->id)
            ->orderBy('sent_at', 'desc')
            ->limit(500)
            ->get();

        return view('myfinance2::dipbuyingalerts.crud.history', [
            'items' => $items,
        ]);
    }
    ...
    {
        $notification = DipBuyingNotification::where('id', $id)
            ->where('user_id', auth()->user()->id)
            ->firstOrFail();

        $notification->delete();

        return redirect()->route('myfinance2::dip-buying-alerts.history')
    ...
    public function bulkAction(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        $affected = DipBuyingNotification::whereIn('id', $ids)
            ->where('user_id', auth()->user()->id)
            ->delete();
